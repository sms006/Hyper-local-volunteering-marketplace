...
function setResHtml(sql, cb){
   // pool.getConnection((err, con)=>{
    ...
    for(var i=0; i<res.length; i++){
        table +='<tr><td>'+ (i+1) +'</td><td>'+ res[i].name +'</td><td>'+ res[i].email +'</td><td>'+ res[i].pincode +'</td></tr>';
    }
    table ='<table border="1"><tr><th>Nr.</th><th>Name</th><th>email</th><th>pincode</th></tr>'+ table +'</table>';
...
let sql ='SELECT name, email, pincode  FROM users ';
